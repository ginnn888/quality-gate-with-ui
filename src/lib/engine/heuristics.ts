import type { FileClassification, ReviewStatus } from "../types";

// Lightweight, dependency-free static analysis used by the "simulation" engine.
// It reads the real uploaded source so the console reacts to actual content
// (upload security_vulnerable.js and it lights up) without any external API.

export interface Finding {
  severity: "critical" | "high" | "medium" | "low" | "info";
  kind: "security" | "logic" | "style";
  message: string;
  remediation?: string;
}
...
export function analyzeFile(name: string, rawCode: string): FileAnalysis {
  const findings: Finding[] = [];
  // All pattern checks run against comment-blanked source to avoid false
  // positives on words like "eval()" appearing in a doc comment.
  ...
  if (/[*/]/.test(code.replace(/\/\/.*$/gm, "").replace(/\/\*[\s\S]*?\*\//g, "")) &&
      /function|=>/.test(code) && !/Object\.is\s*\(/.test(code) && /math|calc|percent|multiply|divide/i.test(name + code)) {
    add({
      severity: "low",
      kind: "logic",
  ...
  const countAtLeast = (levels: Finding["severity"][]) =>
    findings.filter((f) => levels.includes(f.severity)).length;

  let importance: FileClassification["importance"] = "medium";
  ...
  const focus = Array.from(
    new Set(findings.map((f) => (f.kind === "style" ? "logic" : f.kind))),
  );
